...
import com.lesehankoding.library.R
 ...
 * @constructor Create empty View pager transformer
 */
internal class ViewPagerTransformer(
    private val transformType: FlickIntroAnimationType
    ...
    override fun transformPage(page: View, position: Float) {
        when (transformType) {
            FlickIntroAnimationType.Flow -> {
                page.rotationY = position * FLOW_ROTATION_ANGLE
            }
            FlickIntroAnimationType.SlideOver -> transformSlideOver(position, page)
            FlickIntroAnimationType.Depth -> transformDepth(position, page)
            FlickIntroAnimationType.Zoom -> transformZoom(position, page)
            FlickIntroAnimationType.Fade -> transformFade(position, page)
            is FlickIntroAnimationType.Parallax -> {
    ...
    private fun applyParallax(page: View, position: Float) {
        ...
        page.findViewById<TextView>(R.id.txtDescription).translationX = computeParallax(page, position, descriptionPF)
    }
